...
The normative rules the codebase is held to. Architecture docs explain *how* a
mechanism works; this file states *what* must hold regardless of mechanism. When
a doc and a principle disagree, the principle wins.

Principles 1–11 below govern the schema-driven UI and slice composition — the
core of the platform. Two further cross-cutting principles:

- **Explicit over heuristic.** Pass real context; never guess it. Fail loud on
  missing context rather than inferring from names, prefixes, or entity types.
  See [`architecture/errors.md`](architecture/errors.md) and
  [ADR-0003](decisions/0003-explicit-errors-over-heuristics.md).
- **Compose, don't recreate.** Reuse through adoption and override, not
  duplication. The design philosophy is in [`ZEN.md`](ZEN.md).
