...
-- name: WeaveCountProjectsByActor :many
...
WHERE actor_id = ANY(@actor_ids::text[])
GROUP BY actor_id;

-- name: WeaveCountOwnedProjectsByActor :many
-- Same as WeaveCountProjectsByActor but restricted to role = 'owner'.
SELECT actor_id, COUNT(DISTINCT project_id)::int AS count
FROM weave_project_actors
WHERE actor_id = ANY(@actor_ids::text[]) AND role = 'owner'
GROUP BY actor_id;

-- name: WeaveCountMembersByInstitution :many
-- Person-actor count per parent institution, used by actoradmin's
-- institution browse view.
SELECT parent_id, COUNT(*)::int AS count
FROM weave_actors
WHERE type = 'person' AND parent_id = ANY(@institution_ids::text[])
GROUP BY parent_id;

-- name: WeaveGetLastLoginsByActor :many
