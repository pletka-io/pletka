...
const weaveListVocabularySettingsOptions = `-- name: WeaveListVocabularySettingsOptions :many
SELECT
    v.id,
...
func (q *Queries) WeaveListVocabularySettingsOptions(ctx context.Context, projectID string) ([]WeaveListVocabularySettingsOptionsRow, error) {
	...
	defer rows.Close()
	items := []WeaveListVocabularySettingsOptionsRow{}
	for rows.Next() {
		var i WeaveListVocabularySettingsOptionsRow
		if err := rows.Scan(
			&i.ID,
			&i.SystemName,
			&i.UiName,
