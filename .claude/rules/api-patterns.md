...
- `code` — machine-readable branch key; frontend switches on this, not `error` text. Constants: `bad_request`, `unauthorized`, `forbidden`, `not_found`, `conflict`, `validation`, `in_use`, `internal`, `method_not_allowed`, `unprocessable`. Slice-specific additions are rare and must be justified in a comment where the constant is declared; the only one so far is `vocabulary_service_unavailable` (`pkg/weave/settings/handler.go`), a `500` on the settings pane when the configured vocabulary service does not answer — it tells a curator "we could not ask the service" apart from the generic `internal`.
