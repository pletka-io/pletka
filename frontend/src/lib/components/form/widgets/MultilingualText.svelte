...
<div>
  ...
  <!-- Primary input with globe toggle -->
  <div class="flex gap-2">
    <div class="flex-1">
      <input
        ...
        type="text"
        value={value?.[lang] ?? ''}
        oninput={(e) => updateValue(lang, (e.target as HTMLInputElement).value)}
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1">
            <input
              type="text"
              value={value?.[l.code] ?? ''}
              oninput={(e) => updateValue(l.code, (e.target as HTMLInputElement).value)}
              placeholder={l.name}
              class="shadow-sm focus:ring-pletka-primary focus:border-pletka-primary block w-full sm:text-sm border-gray-300 rounded-md"
            />
          </div>
