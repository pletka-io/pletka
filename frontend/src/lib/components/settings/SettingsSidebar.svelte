  ...
  <ul class="space-y-1">
    {#each sections as section (section.id)}
      {#if section.href}
        <li>
          <a href={section.href}
            class="flex items-center gap-3 px-3 py-2 text-sm rounded-md text-gray-700 hover:bg-gray-100 hover:text-gray-900">
            <span class="text-gray-400 w-5 h-5 flex items-center justify-center">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
              </svg>
            </span>
            <span>{tr(section.label, lang)}</span>
          </a>
        </li>
      {:else}
        <li>
          <button
            ...
            class="w-full flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors
              {activeId === section.id
                ? 'bg-pletka-primary/10 text-pletka-primary font-medium'
                : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'}">
            <span class="w-5 h-5 flex items-center justify-center
              {activeId === section.id ? 'text-pletka-primary' : 'text-gray-400'}">
              <!-- Icon placeholder — uses section.icon name for future heroicon integration -->
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                {#if section.icon === 'cog-6-tooth'}
                ...
                {:else if section.icon === 'exclamation-triangle'}
            ...
            <span class="flex-1 text-left">{tr(section.label, lang)}</span>
            ...
            {#if section.placeholder}
              <span class="text-[10px] uppercase tracking-wide text-gray-400">soon</span>
            {/if}
          </button>
        </li>
      {/if}
